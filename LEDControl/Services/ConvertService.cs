...
public class ConvertService : BackgroundService
{
    ...
    public ConvertService(IHubContext<ConvertHub> hubContext, IServiceScopeFactory serviceScopeFactory)
    {
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        {
            var continueProc = await DoWork(stoppingToken);
            if (!continueProc)
    ...
    private async Task<bool> DoWork(CancellationToken token)
    {
        ...
        var videos = dataContext.Videos.Where(p =>
            p.ConvertStatus != ConvertStatus.Done && p.ConvertStatus != ConvertStatus.Failed);
        ...
        foreach (var video in videos)
        {
            ...
            var ytVideo = (await _youtube.GetAllVideosAsync(video.Link))?
                .OrderByDescending(o => o.AudioBitrate).FirstOrDefault();
            if (ytVideo is null)
            {
            ...
            {
                video.ConvertProgress = (int)(Math.Round(args.Duration.TotalSeconds / args.TotalLength.TotalSeconds, 2) * 100);
